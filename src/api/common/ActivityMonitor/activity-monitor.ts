...
import { isElectronStatus, now } from './helpers'
import {
  ActivityMetrics,
...
export const DEFAULT_ACTIVITY_MONITOR_CONFIG: Required<
  Omit<ActivityMonitorConfig, 'logger' | 'electronIpcAdapter' | 'loggerOptions'>
> = {
...
@singleton()
export class ActivityMonitor implements IActivityMonitor {
  public isInitialized: boolean = false
  public status$!: Observable<ActivityStatus>
  private readonly _name: string = 'ActivityMonitor'
  private config!: typeof DEFAULT_ACTIVITY_MONITOR_CONFIG
  // private logger!: Logger<ILogObj>
  private _logger: ILogService<any>
  private _statusSubject!: BehaviorSubject<ActivityStatus>
  private statusInternal$!: Observable<ActivityStatus>
  ...
  configure(config: ActivityMonitorConfig): void {
    ...
    this.electronIpcAdapter = config.electronIpcAdapter
